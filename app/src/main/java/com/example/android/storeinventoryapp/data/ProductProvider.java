...
public class ProductProvider extends ContentProvider {
    ...
    static {
        sUriMatcher.addURI(ProductContract.CONTENT_AUTHORITY, ProductContract.PATH_INVENTORY,
                PRODUCTS);
        sUriMatcher.addURI(ProductContract.CONTENT_AUTHORITY, ProductContract.PATH_INVENTORY +
                "/#", PRODUCT_ID);
    }
    ...
    private Uri insertProduct(Uri uri, ContentValues values) {
        ...
        if (name == null) {
            throw new IllegalArgumentException("Give the product a valid name");
        }

        Integer price = values.getAsInteger(ProductEntry.COLUMN_PRICE);
        if (price == null && price < 0) {
            throw new IllegalArgumentException("The product needs to be given a valid price");
        }
    ...
    private int updateProduct(Uri uri, ContentValues values, String selection, String[]
        ...
        if (values.containsKey(ProductEntry.COLUMN_PICTURE)) {
            ...
            if (picture == null) {
                throw new IllegalArgumentException("You need to select an image for this product");
            }
        }
